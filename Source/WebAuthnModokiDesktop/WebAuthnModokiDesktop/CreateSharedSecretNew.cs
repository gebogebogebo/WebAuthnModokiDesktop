...
namespace gebo.CTAP2
{
    public class CreateSharedSecretNew
    {
        ...
        {
            Debug.WriteLine($"CreateSharedSecretNew-START");
            Debug.WriteLine($"in aG_x={aG_x}");
            Debug.WriteLine($"in aG_y={aG_y}");
...

            {
                ...
                bG_x.Append(gebo.CTAP2.Common.BytesToHexString(pkeyx.ToArray()));
                bG_y.Append(gebo.CTAP2.Common.BytesToHexString(pkeyy.ToArray()));
            }

            {
                ...
                var array = gebo.CTAP2.Common.HexStringToBytes(publicKeyforChgKey);
                ...
                sharedSecret.Append(gebo.CTAP2.Common.BytesToHexString(key));
            }

            Debug.WriteLine($"out bG_x={bG_x}");
            Debug.WriteLine($"out bG_y={bG_y}");
            Debug.WriteLine($"out sharedSecret={sharedSecret}");
            Debug.WriteLine($"CreateSharedSecretNew-END");
